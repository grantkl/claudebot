...
def create_app(config: Config, claude_manager: ClaudeManager, rate_limiter: RateLimiter) -> AsyncApp:
    ...
    async def _handle_message(event: dict[str, Any], say: Any, client: Any) -> None:
        ...
        if superuser:
            mcp_server_names: set[str] = {"sonos", "homekit", "gmail", "calendar", "scheduler", "flights", "flight_watch", "google_flights", "seats_aero", "playwright", "stocks", "web_search", "shopping_list", "deploy"}
        elif authorized:
