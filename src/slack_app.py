...
_SERVER_KEYWORD_RULES: list[tuple[set[str], re.Pattern[str]]] = [
    ...
    ({"fb_marketplace"}, re.compile(
        r"\bfacebook\b|\bmarketplace\b|\bfb\b|\bcraigslist\b"
...
def create_app(config: Config, claude_manager: ClaudeManager, rate_limiter: RateLimiter) -> AsyncApp:
    ...
    async def _handle_message(event: dict[str, Any], say: Any, client: Any) -> None:
        ...
        if superuser:
            mcp_server_names: set[str] = {"sonos", "homekit", "gmail", "calendar", "scheduler", "flights", "flight_watch", "google_flights", "seats_aero", "stocks", "web_search", "shopping_list", "memory", "fb_marketplace"}
        elif authorized:
